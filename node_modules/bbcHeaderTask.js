...
function BbcHeaderTask() {
	...
	/*------------------------------------------------------------------
		Variables
	------------------------------------------------------------------*/
	var taskFlow = [editCss, hideTopHeader, copyBbcLogo, addBbcLogo, addSmallNav];
	...
	function editCss() {
		...
		var newClassesToAdd = [
			...
			"@media(max-width:1100px){.large-nav{display:none !important}}",
			"@media(min-width:1101px){.small-nav{display:none !important}}",
			".section-navigation a:hover, .section-navigation a:visited:hover, .section-navigation a:active, .section-navigation a:visited:active {border-bottom:3px solid #CC0101;}"
		];
	...
	function copyBbcLogo() {

		var bbcLogoPath = model.localAssetsPath + model.imagesPath + "/BBC_NewsLogo.gif";
		var bbcLogoOutputPath = model.outputFolderPath + "/static/img";
		
		//copy the istats lib into the new lib directory
		ncp(bbcLogoPath, bbcLogoOutputPath + "/bbc_newslogo.gif", function (err) {
			if (err) {
	...
	function addBbcLogo() {
		...
		readModuleFile(htmlfilePath, function (err, indexHtmlStr) {
			...
			if (!sectionNavTag.length) {
				//the navigation tag does not already exist, lets create one!
				$('.sections-list').before(navDefualtMarkup);
				sectionNavTag = $('.section-navigation');
			}

			//add the mobile/small screen nav


			var logoMarkup = "<div class=\"bbcTopLogo\">" +
									"<a href=\"http://www.bbc.co.uk/news\" style=\"text-decoration:none; border:none;\">" +
										"<img src=\"./static/img/bbc_newslogo.gif\" alt=\"BBC\" width=\"209\" height=\"24\">" +
									"</a>" +
								"</div>";
	...
	function addSmallNav() {
		...
		readModuleFile(htmlfilePath, function (err, indexHtmlStr) {
			...
			var $ = cheerio.load(indexHtmlStr);
			var sectionNavTag = $('.section-navigation');
			var anchorTags = sectionNavTag.find('a');
			console.log("anchorTags ", anchorTags);

			var a, arrLength = anchorTags.length;
			for (a = 1; a < arrLength; a++) {
				var origAnchorTag = $(anchorTags[a]).addClass('large-nav');
				var anchorHref = anchorTags[a].attribs.href;
				var anchorMarkup = "<a href=\"" + anchorHref + "\" class=\"section-navigator small-nav\">" + a + "</a>";
				$(sectionNavTag).append(anchorMarkup);
			}
