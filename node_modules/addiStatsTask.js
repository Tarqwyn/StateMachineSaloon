...
function AddiStatsTask() {
	...
	var $, el;
	var taskFlow = [checkRequireLoaderBootScriptExists, checkRequireLibExists, copyIstatsLib, editBootScript, addHtmlScript, addCallToStoryJs];
	...
	function editBootScript() {
		...
		readModuleFile(bootScriptPath, function (err, bootScriptStr) {
			...
			var istatsPath = (model.bbcVideosExist) ? "'istats-1'" : "'../../lib/istats'";
			console.log(istatsPath);
	...
	function addHtmlScript() {
		...
		var scriptStr = "<script data-main=\"static/build/js/boot\" src=\"lib/require.js\"></script>\n";
		var scriptStrWithVids = "<script type=\"text/javascript\" src=\"static/build/js/boot.js\"></script>\n";
	...
	function addCallToStoryJs() {
		...
		var jsFilePath = model.outputFolderPath + '/static/build/js/story.min.js';

		readModuleFile(jsFilePath, function (err, storyJsStr) {

			var storyJsStr = storyJsStr.replace('//# sourceMappingURL=story.min.js.map', '');
			var storyJsOutputStr = istatsCallStr.strReplace('{{ STORY.MIN.JS }}', storyJsStr);
