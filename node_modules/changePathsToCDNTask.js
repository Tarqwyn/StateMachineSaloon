...
function ChangePathsToCDNTask() {
	...
	/*------------------------------------------------------------------
		Variables
	------------------------------------------------------------------*/
	var taskFlow = [setYearAndTicketAndEnv, replaceCSSPaths, replaceJSPaths, replaceImagePaths, replaceMRPaths, replaceBootPaths, updateBootDefine, replaceShareViewPaths, replaceShareModelPaths, replaceBootstrapPaths, replaceCSSFontPaths, replaceCSSImgPaths, copyNewsImage, removeCachingVideoReferences];
	...
	function replaceBootPaths() {
		replaceStringInFile('boot.js', 'boot.js?v=2', model.outputFolderPath + '/index.html', function () {
	...
	function replaceDefines(path) {
		...
		readModuleFile(originalHtmlFilePath, function (err, indexHtmlStr) {
			var paths = ['lib/istats', 'lib/event_emitter', 'static/build/js/bootstrap', 'static/build/js/nsshare-model', 'static/build/js/nsshare-view']
			if (err) {
				console.log("err = ", err);
			}
			for (var i = 0; i < paths.length; i++) {
				indexHtmlStr = indexHtmlStr.replace(
					paths[i],
					environ + '/news/special/' + model.yearPublished + '/newsspec_' + model.jiraNumber + '/' + paths[i] + '.js?v=1.1'
				);
			};
