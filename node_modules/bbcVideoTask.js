...
function AddBbcVideoTask() {
	...
	function addBackgroundVideos () {
		loadTheShorthandWebPageWithDOM(function ($) {
			var projectNumber = window.prompt('Jira ticket number please (e.g. 0000)');
			$('.background-overlay video').each(function (i, elm)  {
				if (elm.children[1].name == 'source') {
					$(this).parent().html(renderBackgroundVideoTemplate(getMapIdFromVideoElm(projectNumber, elm)));
				}
			});
	...
	function getMapIdFromVideoElm (projectNumber, elm) {
		...
		return backgroundVideoMapping[projectNumber][videoId]; // map == Media Asset Page
	}
	...
	function addRequireMap() {
		...
		console.log(smpControllerMarkupStr);

		var scriptStrToLookFor = "common." + model.projectNumber + ".js";

		//loop through the script tags and fin the common.{id num}.js script tag
		readModuleFile(model.outputFolderPath + '/index.html', function (err, indexHtmlStr) {
			if (err) {
				console.log("err = ", err);
			}
			...
			for (a = 0; a < arrLength; a++) {
				if (scriptTags[a]) {
					...
					if (indexOfScripStr) {
						$(scriptTag).after(smpControllerMarkupStr);
						break;
					}
				}
			}
