...
function CopyInputIntoOutput() {
	...
	*/
	...
	function deleteNonEmpydirectory(callback) {
		fs.remove(model.outputFolderPath, function (error) {
			if (error) {
				console.error(error);
			}
			fs.ensureDir(model.outputFolderPath, function (error) {
				if (error) {
					console.error(error);
				}
				callback();
	...
	function copyContents() {
		var filter = function (name) {
		  // Stops mp4 or webm files from being copied into output directory
          return name.search(/(\.mp4|\.webm)/) < 0;
        };
		ncp(inputDir, outputDir, {filter: filter}, function (err) {
